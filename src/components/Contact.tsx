...
const Contact = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // enkel validering så vi slipper tomma fält i mailet
    if (!formData.name || !formData.email || !formData.subject || !formData.message) {
      toast({ title: "Komplettera fälten", description: "Fyll i namn, e-post, ämne och meddelande." });
      return;
    }
    ...
    // Öppnar användarens e-postklient (Gmail/Outlook osv.) med förifyllt mejl
    const mailtoLink =
      `mailto:samct86@gmail.com?subject=${encodeURIComponent(safeSubject)}&body=${encodeURIComponent(body)}`;
          ...
          <div className="grid lg:grid-cols-2 gap-12">
            ...
            <Card className="p-8 shadow-medium animate-slide-in">
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label htmlFor="name" className="text-foreground font-medium">Namn *</Label>
                    <Input
                      id="name"
                      type="text"
                      required
                ...
                <div>
                  <Label htmlFor="subject" className="text-foreground font-medium">Ämne *</Label>
                  <Select
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="Partnerskap">Partnerskap</SelectItem>
                      <SelectItem value="Samarbete">Samarbete</SelectItem>
                ...
                <div>
                  <Label htmlFor="message" className="text-foreground font-medium">Meddelande *</Label>
                  <Textarea
                    id="message"
                    required
                    value={formData.message}
                    onChange={(e) => handleInputChange("message", e.target.value)}
                    className="mt-2 min-h-32 focus:border-primary focus:ring-primary"
                    placeholder="Berätta mer om ditt meddelande…"
                  />
                </div>

                <Button type="submit" variant="hero" size="lg" disabled={isSubmitting} className="w-full">
                  {isSubmitting ? "Skickar…" : <>Skicka meddelande <Send className="ml-2 w-4 h-4" /></>}
                </Button>
              </form>
            </Card>
            ...
            <div className="space-y-8 animate-slide-in" style={{ animationDelay: "200ms" }}>
              <Card className="p-8 shadow-soft border-primary/20">
                ...
                <p className="text-muted-foreground mb-6">För snabba frågor eller direkta samtal</p>
                <Button variant="outline" className="w-full border-primary/20 hover:border-primary hover:bg-primary/5" asChild>
              ...
              <Card className="p-8 shadow-soft border-secondary/20">
                ...
                <p className="text-muted-foreground mb-6">Anslut för professionell networking</p>
                <Button
                  variant="outline"
